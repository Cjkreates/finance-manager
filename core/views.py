...
# Restrict admin creation/reset to staff users
...
def edit_income(request, pk):
    income_entry = get_object_or_404(income, pk=pk, user=request.user)
    if request.method == 'POST':
        form = incomeform(request.POST, instance=income_entry)
        if form.is_valid():
            form.save()
            messages.success(request, "Income updated successfully.")
            return redirect('income_list')
    else:
        form = incomeform(instance=income_entry)
...
def delete_income(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('income_list')
    return render(request, 'core/delete_confirm.html', {'object': income_entry, 'type': 'Income'})

@login_required
def edit_expense(request, pk):
    expense_entry = get_object_or_404(expense, pk=pk, user=request.user)
    if request.method == 'POST':
        form = expenseform(request.POST, instance=expense_entry)
        if form.is_valid():
            form.save()
            messages.success(request, "Expense updated successfully.")
            return redirect('expense_list')
    else:
        form = expenseform(instance=expense_entry)
...
def add_expense(request):
    if request.method == 'POST':
        ...
        form = expenseform()
    return render(request, 'core/add_expense.html', {'form': form})
# No file path, for your reference
